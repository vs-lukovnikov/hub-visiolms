...
class ThemeRegistry {
   ...
   *   The machine name of the active theme.
   */
  public function __construct(&$registry, $theme_id) {
   ...
   */
  public function registerPreprocessHooks($theme) {
    foreach (array('preprocess') as $type) {
      // Iterate over all files in the current theme.
      foreach ($this->discoverPreprocessFiles($theme, $type) as $item) {
   ...
   */
  protected function discoverPreprocessFiles($theme, $type) {
